...
El webhook de tipo `newOrder` se activa cuando hay un nuevo pedido en tu página web.

Si este pedido es con un medio de pago online, se activa después de que se realice el pago.

Para aceptar o rechazar el pedido se debe realizar una llamada a `acceptOrder` o `rejectOrder`.

Sólo se activa una vez por pedido.

Si el medio de pago se rechaza no se activa.

### Estructura

Se enviará la información del pedido con el formato Order.
