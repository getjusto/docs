...
El endpoint responde `202 Accepted` cuando el payload fue autenticado, validado y aceptado para procesamiento. La respuesta incluye un `jobId`; guarda ese valor para consultar el resultado final.

```json
{
  "success": true,
  "data": {
    "jobId": "ap-async-job-abc123",
    "status": "queued",
    "service": "adapters",
    "operation": "menu-put-mcd-v1",
    "storeId": "{storeId}",
    "preview": {
      "menus": [
        {
          "menuId": "{storeId}:ADSA-PE-PILOTO SP1-DLV",
          "categories": 13,
          "products": 160,
          "modifiers": 80,
          "options": 1479
...
| Estado | Significado |
| --- | --- |
| `queued` | El payload fue aceptado y el job está esperando ejecución. |
| `processing` | Justo está creando o actualizando menús, productos, modificadores y opciones. |
| `completed` | La carga terminó correctamente. `result` contiene el resumen final, incluyendo `cleanup`. |
| `failed` | La carga falló. Revisa `errorMessage`. |
...
- La llamada valida y acepta el menú completo de McDonald's para el local indicado en `{storeId}`. La carga se procesa de forma asincrónica.
...
curl -X GET \
  "https://api.service.getjusto.com/v3/adapters/jobs/{jobId}" \
  -H "Authorization: Bearer <API_KEY>"
```
