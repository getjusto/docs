...
| `order_info.price.order_price` | `order.totalPrice - order.deliveryFee`. Si alguno de los montos no existe, se usa `0` para ese componente. |
| `order_info.price.items_discount` | `order.totalDiscount`. Si no hay descuento, envía `0`. |
| `order_info.price.others_fees.service_price` | `order.serviceFee`. Si no hay cargo de servicio, envía `0`. |
| `order_info.price.delivery_discount`, `order_info.price.shop_paid_money`, `order_info.price.refund_price`, `order_info.price.store_charged_delivery_price` | `0` cuando no hay monto asociado. |
...
| `order_items[].total_price`, `order_items[].sku_price`, `order_items[].real_price` | Precio Justo correspondiente. Si no hay precio informado, envía `0`. |
| `order_items[].sub_item_list[].app_item_id` | `modifierOption.metadata.id` o `modifierOption.externalId` sin `{storeId}:` |
| `order_items[].sub_item_list[].total_price`, `order_items[].sub_item_list[].sku_price` | Precio de la opción. Si no hay precio informado, envía `0`. |
