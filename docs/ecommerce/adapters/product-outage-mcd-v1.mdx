...
Los campos originales que McDonald's necesita para trazabilidad quedan guardados en `metadata` durante la importación de menú, junto con el `storeId` de Justo. El adapter de agotados usa esos `externalId` ya creados para ubicar productos u opciones.
