...
Payload para crear una orden. Es el único body aceptado por `POST /orders`; el resto de la orden sale del state guardado por Ordering API.

<ResponseField name="idempotencyKey" type="string" required>
  Key estable para crear o reintentar el mismo checkout.
</ResponseField>
