...
Ordering API maneja los términos legales igual que inDrive: `GET /cart` devuelve `data.options.terms.hidden: true` y `data.options.terms.checked: true`, y al crear la orden genera automáticamente las firmas vigentes para los términos pendientes del customer. No agregues un paso manual ni envíes `termSignatures` en el flujo normal.
...
- Custom params estén en `orderParams`. Los términos legales se auto-aceptan y se firman internamente como en inDrive.
