...
La selección queda guardada a nivel customer y app. No tienes que repetirla para cada website. Esto guarda `selectedAddressId`, `selectedPlaceId` y `selectedCityId` en el customer. Cuando leas o recalcules el carrito de cualquier website, Ordering API aplicará ese `selectedPlaceId` internamente.

## 3. Seleccionar website con marketplace

Usa marketplace para mostrar comercios disponibles según ciudad, dirección y búsqueda. Si ya seleccionaste una dirección, puedes enviar `externalCustomerId` y Ordering API resolverá `cityId` y `placeId` desde el customer.

```http
GET /v3/ordering/marketplace/recommendations?externalCustomerId={externalCustomerId}
GET /v3/ordering/marketplace/search?externalCustomerId={externalCustomerId}&query={query}
```

También puedes enviar `cityId` y `placeId` explícitos. Cuando los envías juntos, esos valores ganan sobre la dirección guardada.
...
Obtén el menú del website antes de mostrar productos. Ordering API toma el `menuId` desde las preferences del customer. Si todavía no hay un menú guardado, usa el menú por defecto del website.

```http
GET /v3/ordering/websites/{domain}/categories?externalCustomerId={externalCustomerId}
GET /v3/ordering/websites/{domain}/products?externalCustomerId={externalCustomerId}
GET /v3/ordering/websites/{domain}/products/search?externalCustomerId={externalCustomerId}&query={query}
GET /v3/ordering/websites/{domain}/products/{productId}?externalCustomerId={externalCustomerId}
```
...
Si el customer tiene `selectedPlaceId`, Ordering API lo aplica automáticamente al leer o recalcular el carrito. Si tu app envía un `placeId` explícito en preferencias, ese valor gana para el website actual.
...
Usa checkout-state para datos que no son parte estable del carrito: `billing`, `tip`, `cashAmount`, `gift`, `orderParams`, `termSignatures`, `idempotencyKey`, `meta` y datos equivalentes. También puedes guardar `selectedAddressId` como snapshot del checkout actual, pero la selección reutilizable vive en el customer. Este estado expira automáticamente.
...
- Para delivery, exista una dirección seleccionada en el customer o un `placeId` explícito para el checkout.
...
- Sin comercios en marketplace: si envías `cityId` y `placeId`, revisa cobertura. Si envías `externalCustomerId`, revisa que el customer tenga dirección seleccionada con `selectedPlaceId` y `selectedCityId`.
